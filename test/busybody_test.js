...
describe('Node', function() {
    ...
    describe('publish', function() {
        ...
        it('should be able to contact another node', function(done) {
            ...
            goodNodes[1].publish('node_1:test_event', 'test_param')
        })
        ...
        it('should be able to contact another node and send callbacks', function(done) {
            ...
            goodNodes[1].publish('node_1:test_event', 'test_param', function(a) {
        ...
        it("should fail if the who and what format is not valid ('node_name:remote_event')", function() {
            assert.throws(function() {
                goodNodes[1].publish('node_1-what-the-hell')
            })
            assert.throws(function() {
                goodNodes[1].publish('node_1:too:many:colons')
            })
        })

        it('should fail if the node being told does not exist', function(done) {
            goodNodes[0].publish('me-no-exist:remote_event', function(err) {
    ...
    describe('broadcast', function() {
        ...
        it('should send events to all connected nodes', function(done) {
            async.parallel([
                function(done) {
                    goodNodes[0].on('broadcast_eve', function(param1) {
                        assert.equal(param1, 'test-param')
                        done()
                    })
                },
                function(done) {
                    goodNodes[1].on('broadcast_eve', function(param1) {
                        assert.equal(param1, 'test-param')
                        done()
                    })
                }
            ...
            goodNodes[2].broadcast('broadcast_eve', 'test-param')
        })

        it('should catch events broadcasted very quickly', function(done) {
            async.parallel([
                function(done) {
                    goodNodes[0].on('broadcast_eve', function(param1) {
            ...
            goodNodes[2].broadcast('broadcast_eve', 'test-param 1')
            goodNodes[2].broadcast('broadcast_eve', 'test-param 2')
            goodNodes[2].broadcast('broadcast_eve', 'test-param 3')
        })
    })
    ...
    describe('leave', function() {
        ...
        it('should publish the bridge that it is leaving', function(done) {
