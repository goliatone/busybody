...
class Bridge extends EventEmitter {
            ...
            events.forEach(event => {
                ...
                switch (message.method) {
                    ...
                    case enums.IDENT:
                        ...
                        break;
                    case enums.PUBLISH:
                        self._onPublishMessage(socket, message);
                        break;
                    case enums.BROADCAST:
                        self._onBroadcastMessage(socket, message);
                        break;
                    case enums.RPC:
                        self._onRpcMessage(socket, message);
                        break;
                    case enums.ACK:
    ...
    _onRpcMessage(socket, message) {
        ...
        const payload = {
            ...
            response: function() {
                ...
                that._sendToNode(message.nodeName, new Message(enums.ACK, [message.id, args]));
            }
        };
     ...
     * @param {Message} message Message to serialzie and wire
     */
    _onBroadcastMessage(_, message) {
        const msg = new Message(enums.BROADCAST, message.data);
    ...
    _onPublishMessage(socket, message) {
        ...
        if (!this._sockets.has(nodeName)) {
            ...
            this._sendToNode(nodeName, new Message(enums.PUBLISH, [message.nodeName, message.id, nodeEvent, params]));
        }
    }
    ...
    _sendToNode(name, what) {
        if (this._sockets.has(name)) {
            ...
            debug('tried to write to an unregistered socket', name, what);
        }
    }
