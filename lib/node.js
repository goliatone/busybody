...
class Node extends EventEmitter {
    ...
    publish( /*whoWhat, ...args, callback*/ ) {
        ...
        const msg = new Message(enums.PUBLISH, destination);

        if (callback) {
            this._callbacks[msg.getId()] = callback;
        }
     ...
     */
    broadcast() {
        this._writeBridge(new Message(enums.BROADCAST, [].slice.call(arguments)));
        return this;
    }
    ...
    rpc() {
        ...
        const destination = [rpcMethod, args];

        const msg = new Message(enums.RPC, destination);
    ...
    _connectBridge() {
        ...
        this._bridge = require('./transport').createConnection(this._config);
            ...
            events.forEach(event => {
                ...
                switch (message.method) {
                    case enums.ACK:
                        ...
                        break;
                    case enums.PUBLISH:
                        this._onPublishMessage(message);
                        break;
    ...
    _onPublishMessage(message) {
        var self = this;
        const [from, cbid, eventName, params] = message.data;

        debug('received PUBLISH(%s) from %s', eventName, from);
    ...
    _onBroadcastMessage(message) {
        debug('received BROADCAST(%s)', message.data[0]);
     ...
     */
    _writeBridge(msg) {
        debug('node %s will write to bridge', this._config.id, msg);
        msg.setNodeName(this._config.id);
        try {
            this._bridge.write(msg.toString());
        } catch (e) {
            debug('error writing to bridge', e);
            this.emit('error', e);
        }
    }
